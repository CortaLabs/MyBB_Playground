...
- They will be synced to the database as `readme_test_{template_name}`
- Example: `welcome.html` becomes `readme_test_welcome` in MyBB
...
```php
global $templates;
eval("\$template_html = \"" . $templates->get('readme_test_welcome') . "\";");
```
