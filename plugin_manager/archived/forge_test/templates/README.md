...
- They will be synced to the database as `forge_test_{template_name}`
- Example: `welcome.html` becomes `forge_test_welcome` in MyBB
...
```php
global $templates;
eval("\$template_html = \"" . $templates->get('forge_test_welcome') . "\";");
```
