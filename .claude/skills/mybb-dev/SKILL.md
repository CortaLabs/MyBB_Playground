...
**NEVER connect to the MyBB database directly.** This means:
- NO direct MySQL/MariaDB connections
- NO raw SQL queries outside MCP tools
- NO pymysql, mysql-connector, or any DB libraries
- NO reading `.env` to get credentials for direct access

**ONLY use `mybb_*` MCP tools.** The MCP server handles all database operations internally. If a tool doesn't exist for what you need, ask the user to create one.

This applies to ALL agents (orchestrator, research, coder, etc.) with NO exceptions.
...
- `MCP_TOOLS.md` — Complete tool reference with decision trees (103 tools)
- `CORTEX.md` — Cortex template syntax reference (conditionals, expressions, security)
- `QUICK_REFERENCE.md` — Quick tool patterns (subset of MCP_TOOLS.md)
...
See [MCP_TOOLS.md](MCP_TOOLS.md) for the complete reference with decision trees covering all 103 tools. Most common:
