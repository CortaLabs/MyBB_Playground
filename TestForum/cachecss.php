...
try {
    ...
    // Attempt to re-cache the stylesheet (creates physical CSS files)
    if (!cache_stylesheet($tid, $stylesheet, $style['stylesheet'])) {
        throw new Exception("Failed to cache stylesheet");
    }

    // Rebuild the pre-computed stylesheet list for inheritance resolution
    // This updates themes.stylesheets serialized array
    update_theme_stylesheet_list($tid);

    echo json_encode([
        'success' => true,
        'message' => "Successfully cached stylesheet: $stylesheet for theme: $theme_name (list rebuilt)"
    ]);
